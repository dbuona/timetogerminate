...
\textbf{Project Status:} The experimental procedures are underway, and expected to conclude in December 2018. Time courses from the first four treatment levels of experiment one can be seen in figure~\ref{fig:Figure 6}. As expected, this limited data indicates that stratification accelerates germination rate and increases final germination percentages.\\
\subsection*{Implications}
